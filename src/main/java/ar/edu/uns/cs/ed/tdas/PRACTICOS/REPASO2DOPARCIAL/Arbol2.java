...
public class Arbol2<E> implements Tree<E>{
        ...
        //Métodos
            ...
            public boolean eliminarUltimo(Position<E> p) throws InvalidPositionException{
                ...
                if(nodo == root){
                    ...
                    PositionList<TNodo<E>> lista = nodop.getHijos();
                    PositionList<TNodo<E>> listah = nodo.getHijos();
                    if (lista.isEmpty()) {
                        return false;
                    }

                    Position<TNodo<E>> aeliminar = lista.last();
                    if(aeliminar.element() == nodo){
                        for(TNodo<E> node:listah){
                            lista.addBefore(aeliminar, node);
                            node.setPadre(nodop);
                        }
                        lista.remove(aeliminar);
                        aeliminar.element().setPadre(null);
                        while(!listah.isEmpty()){
                            listah.remove(listah.last());
                        }
                        retorno = true;
                        cantelem--;
                    }
