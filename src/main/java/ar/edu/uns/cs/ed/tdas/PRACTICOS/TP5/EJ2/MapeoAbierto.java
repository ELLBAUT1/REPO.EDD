...
public class MapeoAbierto<K,V> implements Map<K,V>{
    ...
    //Servicios
        ...
        //Métodos
            //Comandos
            public V put(K key, V value) throws InvalidKeyException{ //normalmente constante, en el peor caso de orden n si se llama al rehash
                    checkKey(key);
            ...
            public V remove(K key) throws InvalidKeyException{ // orden constante
                checkKey(key);
                int indice = hashCodeMapa(key);
                PositionList<Entry<K,V>> listain = hashmap[indice];
            ...
            public V get(K key) throws InvalidKeyException{ //orden constante
                checkKey(key);
                int indice = hashCodeMapa(key);
                PositionList<Entry<K,V>> listain = hashmap[indice];
                for(Position<Entry<K,V>> pos: listain.positions()){
            ...
            public Iterable<K> keys(){ //orden n
            ...
            public Iterable<V> values(){ //orden n
            ...
            protected int hashCodeMapa(K clave){ //orden constante
            ...
            protected int proximoPrimo(int capacidadActual) { //orden n
            ...
            protected K checkKey(K clave) throws InvalidKeyException{ //orden constante
                if(clave == null){
